...
class ReportGeneralLedger(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account,coa_ids,company_id):
        ...
        if init_balance:
            ...
            sql = ("""SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode, 0.0 AS amount_currency, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) as balance, '' AS lpartner_id,\
                            ...
                            JOIN account_journal j ON (l.journal_id=j.id)\
                            WHERE  m.company_id= %s and m.state != 'cancel' and l.parent_state != 'cancel' and l.account_id = %s""" + filters + '  GROUP BY l.account_id')
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, to_char(l.date, 'dd/mm/yyyy') AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
            ...
            JOIN account_journal j ON (l.journal_id=j.id)\
            JOIN account_account acc ON (l.account_id = acc.id) \
            WHERE m.company_id=%s and m.state != 'cancel' and l.parent_state != 'cancel'  and l.account_id = %s ''' + filters + ''' GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.ref, l.name, m.name, c.symbol, p.name ORDER BY ''' + sql_sort)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if not self.model:
            self.model = data.get('active_model')


        docs = self.env[self.model].browse(self.env.context.get('active_ids', []))
        if not docs:
            docs = self.env[self.model].browse(data.get('active_ids', []))
